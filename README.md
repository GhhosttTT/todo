...
- Scheduled：未来有日期的任务。
- All：全部任务。
- 新增、编辑、日期、提醒时间、完成/恢复、删除撤销、拖动排序和搜索。
- SF Pro 字体与 Windows 中文字体 fallback。
- 白色、黑色两套完整主题，可在设置中切换并持久化。
...
任务编辑里可以设置具体提醒时间。应用不会每分钟轮询，而是在主进程为未来提醒注册一次性系统通知；应用关闭或电脑睡眠期间错过的提醒，会在下次启动或数据刷新时补发一次。
...
- 英文、数字和 Latin 标点来自用户提供的 `SF-Pro.dmg`，构建资产已裁成四个 WOFF2 字重。
- SF Pro 不含中文字形；中文采用 Noto Sans SC 可变字重子集，并在可用时优先系统苹方。
- 中文字体资产只保留中文、英文、数字、中文标点和全角字符，不包含假名或韩文。
- Noto Sans CJK 使用 SIL Open Font License，许可文本位于 `assets/fonts/OFL-NotoSansCJK.txt`。
