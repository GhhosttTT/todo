...
| All | 全部未完成任务；今天及逾期任务排在最上方 | 全部任务；未完成任务仍优先，今天及逾期任务排在未完成任务最上方 |

默认选中 All。无截止日期任务只出现在 All。搜索始终在当前视图结果内匹配标题和备注，忽略大小写并去除查询首尾空格。空查询恢复当前视图。

同一任务只存一份，由过滤规则进入多个视图；Today 是 Scheduled 的互斥日期区间，但二者都属于 All。All 内部排序按“未完成优先、今天及逾期、未来、有日期为空”的顺序分组，分组内继续保留用户手动排序。三个侧栏计数始终只统计未完成任务，不随“显示已完成”开关变化。
...
- 查看模式只读：任务完成按钮仅作状态图形，不响应任务变更；拖动、搜索和编辑控件隐藏，不提供窗口内编辑按钮。侧栏底部只显示不可点击的全局快捷键提示，让用户知道通过快捷键进入编辑。
- 普通应用窗口应覆盖它，符合“桌面最底层”。
...
- `BrowserWindow` 不应再把 `height`、`minHeight`、`maxHeight` 统一锁死为 `620`；`620` 只作为默认高度。
- 窗口控制层应拆分默认尺寸和最小尺寸，例如 `DEFAULT_LAYOUT_DIMENSIONS` 与 `MIN_LAYOUT_DIMENSIONS`。
- `safeBounds` / `clampBounds` 必须保留用户保存的 `width` 和 `height`，只负责限制最小尺寸、最大工作区和屏幕可见性。
- 进入编辑模式时调用 `setResizable(true)` 和 `setMinimumSize()`；退出编辑模式前保存当前 bounds，再调用 `setResizable(false)`。
- `TaskStore.parseWindowBounds` 必须保留合法的持久化宽高，不能用布局默认尺寸覆盖用户自定义尺寸。
- `applyLayoutMode` 切换紧凑 / 展开时必须使用目标布局的 `compactWindowBounds` 或 `windowBounds`，不能把默认尺寸强行覆盖进去。
- `move` / `resize` 事件只在编辑模式保存，保存时使用窗口真实 bounds；查看模式下的 WorkerW 重绑、显示器变化校正不应污染用户主动设置的尺寸，除非原 bounds 已不可见。
- 如果 frameless 原生边缘 resize 在 Windows 上表现不稳定，第一后备方案是 renderer 显示 8 个自定义 resize handle，并通过 IPC 让主进程按鼠标屏幕坐标设置 bounds；不得只做 CSS `resize`。
...
- 展开模式默认尺寸为 `900 × 620`：左侧统计导航，右侧任务区域。
- 紧凑模式默认尺寸为 `400 × 620`：顶部横排 Today、Scheduled、All 三个统计入口，下方为标题和任务区域。
- 紧凑 / 展开是布局预设，不是永久固定尺寸；两种布局分别保存自己的窗口位置和用户拖拽后的尺寸。
- 只有编辑模式允许通过窗口边缘或右下角拖拽调整大小；查看模式不显示 resize 暗示，也不响应尺寸拖拽，但沿用最近一次保存的尺寸展示。
- 进入编辑模式时窗口脱离 WorkerW 并启用 resize；退出编辑模式时保存当前 bounds、禁用 resize，并用当前 bounds 重新绑定桌面层。
- 最小尺寸建议：展开 `720 × 460`，紧凑 `360 × 480`。最大尺寸约束到当前显示器工作区，显示器、DPI 或任务栏变化后必须校正到可见范围内。
- 紧凑模式不裁剪功能或任务字段。任务标题、备注、截止日期和提醒时间允许完整换行，任务区独立滚动；设置页使用全宽覆盖层并独立滚动。
- 任务数量和展开编辑表单只能改变内部滚动内容，不能撑大窗口内容结构或把底部控制区顶出可见区域。
...
- 默认窗口为展开模式 `900 × 620`；编辑态可拖拽改变宽高，查看态保持保存后的宽高。
- 侧栏宽度 238px；小于 760px 时收窄为 196px，不隐藏固定入口。
- 顶部保留窗口拖动区，交互控件设置 `-webkit-app-region: no-drag`。
- 编辑态右下角显示克制的 resize grip；白色和黑色主题下都必须可见但不抢占视觉焦点。
- 侧栏固定三个入口，不显示 My Lists、Add List 或 Flagged。
- 查看模式隐藏搜索框、编辑工具和窗口内编辑入口，只保留列表、任务内容和快捷键提示。
...
- 默认快捷键：`Ctrl+Alt+T`。
- 首次注册失败时状态为“未激活”，托盘仍是可靠入口。用户聚焦快捷键录制框时临时暂停旧快捷键，避免录制当前组合键时触发模式切换；点击“检测并应用”后实际向 Windows 注册新组合键，以此检测无效按键或被其他程序占用。注册或设置保存失败时恢复旧快捷键和设置值。
- 快捷键触发：查看/隐藏状态进入编辑；编辑状态退出前必须执行未保存表单保护，再回到桌面层。
- 托盘菜单的模式动作互斥：桌面状态只显示“打开编辑”，编辑状态只显示“回到桌面”；其余保留重试桌面绑定和退出。
- 单击托盘图标切换编辑模式；菜单提供确定性操作。
- 托盘资源缺失时应用仍运行，并记录降级；开发版可使用程序生成的简洁模板图标。
- 开机启动默认关闭；用户开启后通过 Electron 登录项接口写入 Windows 启动配置。单文件 portable 包必须使用 `PORTABLE_EXECUTABLE_FILE` 指向用户实际持有的启动器，不能记录解压后的临时 `process.execPath`；普通打包环境才使用当前进程路径。
...
| 编辑态拖拽窗口尺寸 | 只在编辑模式响应 resize；退出编辑后保存当前布局 bounds 并回到桌面层 |
| 查看态误触窗口边缘 | 不响应 resize，不显示 resize grip，保持低打扰桌面插件状态 |
| 紧凑 / 展开切换 | 使用目标布局自己的保存尺寸；没有历史尺寸时使用默认尺寸 |
| 保存尺寸超出当前屏幕 | clamp 到当前工作区内，优先保留用户宽高，必要时按最大可见范围收缩 |
...
- 视图：默认选中 All；Today 包括今天和逾期；Scheduled 只含未来；All 含所有活动任务，且今天及逾期任务排在最上方。
...
8. 编辑模式可以拖拽窗口宽高；查看模式不能误触 resize；重启、退出编辑和切换紧凑 / 展开后保留对应布局的保存尺寸。
9. 普通和便携路径解析可测试；便携不可写时降级到普通目录并显示状态。
10. 四个 SF Pro 拉丁字符 WOFF2 和一个简体中文 WOFF2 子集从 `assets/fonts` 加载；系统有苹方时中文优先使用苹方，任一字体缺失或损坏时使用中文友好的系统 fallback，文本不溢出或遮挡。
11. 白色、黑色主题均可在设置中切换并持久化；常规尺寸与最小尺寸下无文本重叠、控制溢出或不可辨识状态。
12. UI 不出现自定义列表、Flagged、账号、云同步或置顶设置。
13. `npm test`、`npm run typecheck` 和 `npm run build` 通过。
