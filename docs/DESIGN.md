...
- 查看模式与编辑模式。
- 任务新增、编辑、完成、恢复、删除、撤销删除、排序、日期和搜索。
- 本地 JSON 持久化、普通数据模式和便携数据模式。
- 托盘、全局快捷键、窗口状态记忆。
- WorkerW 桌面层绑定及可观察的降级行为。

第一版不实现账号、云同步、自定义列表、Flagged、附件、复杂重复任务、自然语言日期解析或置顶模式。
...
- 查看模式只读：任务完成按钮仅作状态图形，不响应任务变更；拖动、搜索和编辑控件隐藏，只保留底部编辑入口。
- 普通应用窗口应覆盖它，符合“桌面最底层”。
...
- 查看模式隐藏搜索框和编辑工具，只保留列表、任务和底部小型编辑入口。
...
- 日期：午夜、时区变化和本地日期解析。
- schema：默认状态、v1 校验、未来版本只读、损坏主文件读取备份。
