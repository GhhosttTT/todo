...
- 展开模式固定为 `900 × 620`：左侧统计导航，右侧任务区域。
- 紧凑模式固定为 `400 × 620`：顶部横排 Today、Scheduled、All 三个统计入口，下方为标题和任务区域。
- 两种布局分别保存窗口位置，切换时按目标显示器工作区校正，不能落到屏幕外。
- 紧凑模式不裁剪功能或任务字段。任务标题、备注、截止日期和提醒时间允许完整换行，任务区独立滚动；设置页使用全宽覆盖层并独立滚动。
- 两种布局都保持固定尺寸，任务数量和展开编辑表单只能改变内部滚动内容，不能撑大窗口。
...
- 默认窗口 900 × 620，窗口高度固定为 620；任务数量增加时只滚动任务区，不改变窗口高度。
